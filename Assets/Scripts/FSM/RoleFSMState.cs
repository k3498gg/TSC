...
/// </summary>
public abstract class RoleFSMState
{
    private static EnumComparer<RoleTransition> fastComparer = new EnumComparer<RoleTransition>();
    protected Dictionary<RoleTransition, RoleStateID> map = new Dictionary<RoleTransition, RoleStateID>(fastComparer);
    protected RoleStateID stateID;
    public RoleStateID ID { get { return stateID; } }
    ...
    /// NPC是被该类约束下对象的一个引用
    /// </summary>
    public abstract void OnExcute(Entity entity);
...
/// </summary>
public class RoleFSMSystem
{
    private List<RoleFSMState> states;
    //通过预装一个过渡的唯一方式来盖面状态机的状态
    //不要直接改变当前的状态
    private RoleStateID currentStateID;
    private RoleFSMState currentState;

    public RoleStateID CurrentStateID
    {
        private set { currentStateID = value; }
        get { return currentStateID; }
    }
    public RoleFSMState CurrentState
    {
        private set { currentState = value; }
        get { return currentState; }
    }

    public RoleFSMSystem()
    {
        if (null == states)
        {
            states = new List<RoleFSMState>();
        }
        else
        {
            states.Clear();
